...
class Dictogram(dict):
    ...
    def add_count(self, word, count=1):
        ...
        if word in self:
            # If the word is already in the histogram, increase its count
            self[word] += count
        else:
            # If the word is new, add it to the histogram with its count
            self[word] = count
            # Increase the number of distinct word types
            self.types += 1
